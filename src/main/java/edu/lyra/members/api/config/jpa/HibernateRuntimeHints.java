 ...
 * {@code GraalVMStaticFeature} doesn't touch any of the four categories below). Hibernate reaches every one of them
 ...
 *
 * <p>{@code UUID[]}: every entity persister's {@code MultiIdEntityLoaderArrayParam} reflectively instantiates an
 * array of the entity id type - {@code UUID} for every entity in this application - while preparing that
 * persister's loaders, itself an unconditional step of {@code SessionFactoryImpl} construction; left unregistered,
 * that fails with {@code MissingReflectionRegistrationError: Cannot reflectively instantiate the array class
 * 'java.util.UUID[]'}.
 *
